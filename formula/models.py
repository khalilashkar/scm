...
class Support_descrption(models.Model):
    suppo = models.CharField(max_length=255, null=True,
                             blank=True, verbose_name="اسم الجهة الداعمة ")
    suppo_description = models.CharField(
        max_length=255, null=True, blank=True, verbose_name="وصف حول الجهة ")
...
class Registration(models.Model):
    ...
    city_CHOICES = (
        ('aleppo', 'Aleppo'),
        ('damascus', 'Damascus'),
        ('suburb of damascus', 'Suburb of Damascus'),
        ('daraa', 'Daraa'),
    ...
    # TABLE START
    Application_number = models.AutoField(
        primary_key=True, default=None, unique=True)
    Application_date = models.DateField(
        _("application_date "), null=True, auto_now_add=True)

    # INFORMATION PERSO
    gender = models.CharField(
        _("الجنس"), max_length=30, choices=gender_CHOICES, null=True)
    ...
    phone_state_inf = models.CharField(
        max_length=255, blank=True, choices=bool_CHOICES, default=0, null=True, verbose_name="هل لديك رقم هاتف ؟ ")
    phone = models.CharField(max_length=255, blank=True,
                             null=True, verbose_name="الهاتف ")

    facebook_state_inf = models.CharField(
        max_length=255, blank=True, choices=bool_CHOICES, default=0, null=True, verbose_name="هل لديك حساب فيسبوك ؟ ")
    facebook = models.CharField(
        max_length=255, blank=True, null=True, verbose_name="حساب الفيسبوك")

    # current adress
    country_rec = CountryField(
        max_length=255, verbose_name='الدولة الحالية', default='some_value')
    current_city = models.CharField(
        max_length=255, verbose_name="المدينة الحالية?", default='')

    country = CountryField(max_length=255, verbose_name=" الدولة")
    city = models.CharField(max_length=255, null=True,
                            choices=city_CHOICES, verbose_name="المحافظة")
    region = models.CharField(
        max_length=255, null=True, verbose_name="المنطقة")

    # family state
    family_state = models.CharField(
        max_length=30, choices=family_CHOICES, null=True, default='عازب', verbose_name="الوضع العائلي")

    have_kids = models.CharField(max_length=255, blank=True, default=0,
                                 choices=bool_CHOICES, null=True, verbose_name="هل لديك أولاد؟")
    number_kids = models.CharField(
        max_length=255, blank=True, null=True, verbose_name="كم عدد أفراد العائلة")
    summary_of_recsituation = models.TextField(
        blank=True, null=True, verbose_name="يرجى شرح الوضع الحالي")

    # MEDICAL STATUS
    medical_state_inf = models.CharField(
        max_length=255, blank=True, choices=app_CHOICES, default=1, null=True, verbose_name="هل لديك وضع صحي خاص ")
    medical_note_inf = models.TextField(
        max_length=1000, blank=True, null=True, verbose_name="شرح مختصر لحالتك الصحية ")

    #education and jobs
    educatton_level = models.CharField(
        max_length=255, blank=True, choices=education_CHOICES, null=True, verbose_name="التحصيل العلمي")

    # JOB
    job = models.CharField(max_length=255, blank=True,
                           null=True, choices=jobـCHOICES, verbose_name="المهنة ")

    org_memeber = models.CharField(max_length=255, blank=True, default=1, choices=app_CHOICES,
                                   null=True, verbose_name="هل أنت عضو في أي تجمع إعلامي، أو من الموقعين على مواثيق شرف إعلامية أو سواه من المبادرات المشتركة")
    details = models.TextField(
        max_length=4000, blank=True, null=True, verbose_name="يرجى ذكر التفاصيل ")
    experience = models.CharField(max_length=100, blank=True, choices=bool_CHOICES,
                                  null=True, verbose_name="هل سبق عملت بأجر أو لديك خبرات سابقة ؟")

    # //// DELETED FIELDS //// REPLACE BY "WORK DETAIL"
    # start_date = models.DateField(verbose_name='تاريخ بدء العمل', null=True)
    # current_org_comp = models.CharField(
    #     max_length=255, blank=True, null=True, verbose_name="جهة العمل الحالية")
    # Previous_employers = models.CharField(
    #     max_length=255, blank=True, null=True, verbose_name="جهة العمل السابقة")
    # paid_job = models.CharField(max_length=255, blank=True, null=True,
    #                             default=1, choices=app_CHOICES, verbose_name="هل عملت بإجر مسبقاُ")
    # name_of_company_paid = models.CharField(
    #     max_length=255, blank=True, null=True, verbose_name="اذكر اسم الجهة او المنظمة ")

    # violation information and details about it

    if_article_linke = models.CharField(max_length=100, blank=True, null=True,
                                        verbose_name="هل لديك منشورة باسمك الصريح أو المستعار ؟", choices=bool_CHOICES)
    articls_link_1 = models.TextField(
        max_length=200,  blank=True, null=True, verbose_name="يرجى وضع الروابط اﻹعلامية المنشورة")

    if_stop_work = models.CharField(max_length=100, null=True, blank=True,
    ...
    # :::: مصادر تثبت العمل  :::::::
    resource_prof = models.CharField(max_length=100, blank=True, null=True,
    ...
    email_2 = models.EmailField(
        max_length=255, blank=True, null=True, verbose_name="بريد الكتروني للمصدر الثاني")

    # :::::::::::::: اﻹنتهاكات :::::::::::::
    violations = models.CharField(max_length=255, blank=True, default=1, null=True,
                                  choices=app_CHOICES, verbose_name=" هل سبق أن تعرضت لأي نوع من أنواع الانتهاكات؟")

    relation_with_org = models.CharField(max_length=255, blank=True, choices=app_CHOICES, null=True,
                                         verbose_name="هل لديك أي ارتباطات تنظيمية مع أي فصيل عسكري أو ديني أو تجمع سياسي أو ديني؟")
    summary_of_relations = models.TextField(
        blank=True, null=True, verbose_name="الرجاء الإجابة مع ذكر التفاصيل")

    # ask_for helpe
    type_of_dmande = models.CharField(
        max_length=30, verbose_name="طبيعة المساعدة المطلوبة", choices=demand_CHOICES, null=True, blank=True)
    resaon_for_help = models.TextField(
        blank=True, null=True, verbose_name="رجاء اكتب ملخص لسبب طلب المساعدة ")
    list_of_tools = models.TextField(max_length=255, blank=True, null=True,
                                     verbose_name="إن كان الدعم المطلوب متعلق بمستلزمات خاصة بالعمل، نرجو تزويدنا بقائمة الأسعار")
    last_job_salary = models.CharField(
        max_length=255, blank=True, null=True, verbose_name="يرجى ذكر آخر عمل تقاضيت منه أجر وقيمة الأجر")
    reason_stopping_job = models.CharField(
        max_length=255, blank=True, null=True, verbose_name="لماذا لا تستطيع أن تعمل بأجر في الوقت الحالي؟")
    summary_of_help = models.TextField(
        max_length=2000, blank=True, verbose_name="يرجى شرح الوضع الحالي")

    # other org to helpe
    other_org_demand = models.CharField(max_length=255, blank=True, choices=app_CHOICES,
                                        null=True, verbose_name="هل تقدمت بطلب مساعدة لأي منظمة سابقاً؟ ")
    name_org = models.CharField(
        max_length=255, blank=True, null=True, verbose_name="ما هي المنظمة أو المنظمات")
    date_of_demand_org = models.DateField(
        blank=True, null=True, verbose_name="نرجو معرفة تاريخ تقديم الطلب ؟ ")
    tyoe_of_demand_other_org = models.CharField(
        max_length=255, blank=True, null=True, verbose_name="ما هي طبيعة الطلب؟")
    result_of_demand_other_org = models.CharField(
        max_length=255, blank=True, null=True, verbose_name="ما هي نتيجة الطلب؟")
    know_support_programme = models.CharField(
        max_length=255, blank=True, null=True, verbose_name="كيف علمت ببرنامج الدعم ؟")

    training_media = models.CharField(max_length=255, blank=True, choices=app_CHOICES, default=1, null=True,
                                      verbose_name="هل سبق أن شاركت بأي ورشات أو دورات لتطوير الخبرات الإعلامية أو الحقوقية؟")
    details_traning_media = models.CharField(
        max_length=255, blank=True, null=True, verbose_name="يرجى ذكر التفاصيل ")
    # user name and created by and state check
    created_by = models.ForeignKey(
        User, null=True, blank=True, on_delete=models.CASCADE)
    state_step = models.CharField(
        _("المعالجة"), max_length=30, blank=True, default=0, choices=procesc_CHOICES, null=True)
    support_org_state_1 = models.ForeignKey(
        Support_descrption, null=True, default=1, blank=True, on_delete=models.CASCADE, verbose_name="الجهة المحولة")
...
class WorkDetail(models.Model):
    ...
    registration = models.ForeignKey(
        Registration, null=False, related_name="registration", on_delete=models.CASCADE, default=None)
...
class CaseFile(models.Model):
    ...
    case = models.ForeignKey(Registration, on_delete=models.CASCADE)
...
class SupportOrg(models.Model):
    ...
    date_of_response = models.DateField(
        verbose_name="تاريخ الإحالة ", blank=True)
    result_of_org = models.CharField(
        max_length=255, null=True, choices=result_of_org_CHOICES, default=False, verbose_name="النتيجة")
...
class Violation(models.Model):
    ...
    responsibility = models.CharField(max_length=100, choices=responsibility_CHOICES,
